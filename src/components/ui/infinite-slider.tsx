...
export function InfiniteSlider({
    ...
    // Clone children to add unique keys for duplicated sets
            ...
            >
                {childArray.map((child, i) =>
                    isValidElement(child)
                        ? cloneElement(child, { key: `original-${i}` } as Record<string, unknown>)
                        : child
                )}
                {childArray.map((child, i) =>
                    isValidElement(child)
                        ? cloneElement(child, { key: `duplicate-${i}` } as Record<string, unknown>)
                        : child
                )}
            </motion.div>
        </div>
